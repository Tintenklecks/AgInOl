...
export function IPadLandscape({ src, alt = "", style, hideEmpty }: FrameProps) {
  const resolved = img(src);
  return (
    <div style={{ position: "relative", aspectRatio: "4 / 3", ...style }}>
      <div
        style={{
          width: "100%",
          height: "100%",
          borderRadius: "3.2% / 4.2%",
          background: "linear-gradient(160deg, #2C2C2E 0%, #17171A 100%)",
          position: "relative",
          overflow: "hidden",
          boxShadow: "inset 0 0 0 1px rgba(255,255,255,0.1), 0 18px 70px rgba(0,0,0,0.62)",
        }}
      >
        <div
          style={{
        ...
        >
          {resolved ? (
            <img src={resolved} alt={alt} style={{ display: "block", width: "100%", height: "100%", objectFit: "cover", objectPosition: "top" }} draggable={false} />
          ) : hideEmpty ? null : (
            <EmptySlot />
          )}
        </div>
...
export function MacPanel({ src, alt = "", style, hideEmpty }: FrameProps) {
  const resolved = img(src);
  return (
    <div
      style={{
    ...
    >
      {resolved ? (
        <img src={resolved} alt={alt} style={{ display: "block", width: "100%", height: "100%", objectFit: "contain" }} draggable={false} />
      ) : hideEmpty ? null : (
        <EmptySlot />
      )}
    </div>
