...
function getFrameAspect(device: Device, orientation: Orientation) {
  switch (device) {
    ...
    case "ipad":        return orientation === "landscape" ? 4 / 3 : IPAD_RATIO;
    case "android-7":
  ...
  switch (device) {
    ...
    case "ipad":
      if (orientation === "landscape") {
        return { Comp: IPadLandscape, widthFn: (cW, cH) => tabletLW(cW, cH, 0.72), smallWidthFn: (cW, cH) => tabletLW(cW, cH, 0.56) };
      }
...
function SlideElements({
  ...
  function renderDevice(id: "device" | "deviceSecondary", rect: Rect, src: string, extraStyle?: React.CSSProperties) {
    ...
    const isMacPanel = id === "deviceSecondary" && slide.secondaryVisual === "mac-panel";
    const Visual = isMacPanel ? MacPanel : Frame;
    const visualAspect = isMacPanel ? 796 / 685 : frameAspect;
    return (
      <Movable
        rect={toGlobal(rect)}
        boundsW={boundsW}
